...
const App: React.FC = () => {
  ...
  if (!currentLab) {
    return (
      <div className={`min-h-screen w-full transition-colors duration-500 ${theme === 'dark' ? 'bg-[#020617] text-white' : 'bg-blue-50/40 text-slate-900'} p-12 overflow-y-auto`}>
        <div className="max-w-6xl mx-auto">
          ...
          <header className="mb-20 text-center">
            <h1 className={`text-7xl font-black mb-6 bg-gradient-to-r ${theme === 'dark' ? 'from-blue-400 via-indigo-400 to-emerald-400' : 'from-blue-600 via-indigo-500 to-cyan-500'} bg-clip-text text-transparent tracking-tighter`}>
              Virtual Ohm Lab
            </h1>
            <p className={`${theme === 'dark' ? 'text-slate-400' : 'text-slate-500'} text-xl max-w-2xl mx-auto font-medium leading-relaxed`}>
              Step into your high-fidelity electronics workstation. Learn the physics of circuits with zero risk.
            </p>
            <button 
              onClick={() => setKnowledgeHubOpen(true)}
              className={`mt-8 px-8 py-4 transition-all flex items-center gap-3 mx-auto rounded-2xl text-sm font-black uppercase tracking-widest ${theme === 'dark' ? 'bg-white/5 hover:bg-white/10 border border-white/10' : 'bg-white text-blue-600 hover:bg-blue-50 border border-blue-100 shadow-lg shadow-blue-500/5'}`}
            >
              📚 Knowledge Hub & Ohmie AI
            </button>
          </header>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {LAB_EXERCISES.map((lab) => (
              <div 
                ...
                className={`group relative transition-all cursor-pointer overflow-hidden shadow-2xl p-12 rounded-[3.5rem] border-2 ${theme === 'dark' ? 'bg-slate-900/40 border-white/5 hover:bg-slate-900/80 hover:border-blue-500/30' : 'bg-white border-white hover:border-blue-300 hover:shadow-blue-500/10'}`}
              >
                ...
                <div className="relative z-10">
                  ...
                  <button className={`flex items-center gap-4 text-white font-black text-sm px-8 py-4 rounded-2xl transition-all ${theme === 'dark' ? 'bg-blue-600 hover:bg-blue-500 hover:shadow-[0_0_40px_rgba(37,99,235,0.4)] shadow-black' : 'bg-blue-600 hover:bg-blue-700 shadow-lg shadow-blue-500/30'}`}>
                    Begin Lab Module <span>→</span>
                  </button>
                </div>
              </div>
            ))}
