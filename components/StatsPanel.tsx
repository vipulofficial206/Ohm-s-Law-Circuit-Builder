...
const StatsPanel: React.FC<StatsPanelProps> = ({ selectedComponent, onUpdateComponent, onDeleteComponent, totalStats, theme }) => {
  const [viewMetric, setViewMetric] = useState<'current' | 'power' | 'both'>('current');

  const handleDownload = () => {
    if (!selectedComponent) return;
    
    const timestamp = new Date().toISOString().replace(/[:.]/g, '-');
    const header = "Sample,Time(ms),Current(mA),Power(mW)\n";
    const rows = Array.from({ length: 24 }).map((_, i) => {
      const time = i * 4;
      const current = (selectedComponent.current * 1000).toFixed(2);
      const power = (selectedComponent.power * 1000).toFixed(2);
      return `${i + 1},${time},${current},${power}`;
    }).join("\n");
    ...
    a.setAttribute('download', `lab_data_${selectedComponent.type}_${timestamp}.csv`);
            ...
            <div className={`p-6 rounded-2xl border space-y-4 shadow-inner ${theme === 'dark' ? 'bg-slate-950/50 border-white/5' : 'bg-slate-100 border-slate-200'}`}>
              <div className="flex justify-between items-center mb-2">
                <h5 className={`text-[10px] font-black uppercase tracking-widest flex items-center gap-2 ${theme === 'dark' ? 'text-slate-500' : 'text-slate-400'}`}>
                  <span className={`w-2 h-2 rounded-full animate-pulse ${viewMetric === 'current' ? 'bg-emerald-500' : viewMetric === 'power' ? 'bg-indigo-500' : 'bg-amber-500'}`}></span>
                  Waveform
                </h5>
                <button 
                  onClick={handleDownload}
                  className={`p-2 rounded-lg transition-all ${theme === 'dark' ? 'hover:bg-white/5 text-slate-500 hover:text-white' : 'hover:bg-slate-200 text-slate-400 hover:text-slate-900'}`}
              ...
              <div className="flex gap-2 mb-4">
                {(['current', 'power', 'both'] as const).map(m => (
                  <button
                    key={m}
                    onClick={() => setViewMetric(m)}
                    className={`flex-1 py-1.5 text-[8px] font-black uppercase tracking-widest rounded-lg border transition-all ${
                      viewMetric === m 
                      ? 'bg-blue-600 border-blue-600 text-white shadow-md' 
                      : theme === 'dark' ? 'bg-white/5 border-white/5 text-slate-600 hover:text-slate-400' : 'bg-white border-slate-200 text-slate-400 hover:bg-slate-50'
                    }`}
                  >
                    {m}
                  </button>
              ...
              <div className="h-32 w-full flex items-end gap-[1px] overflow-hidden px-1 relative">
                {/* Horizontal Baseline */}
                <div className={`absolute bottom-0 left-0 right-0 h-[1px] ${theme === 'dark' ? 'bg-white/10' : 'bg-slate-300'}`}></div>
                
                {Array.from({ length: 24 }).map((_, i) => {
                  const currentHeight = Math.max(4, (selectedComponent.current * 1000) * (0.6 + Math.sin(i * 0.5) * 0.1));
                  const powerHeight = Math.max(4, (selectedComponent.power * 1000) * (0.6 + Math.cos(i * 0.5) * 0.1));
                    ...
                    <div key={i} className="flex-1 flex flex-col items-center justify-end h-full group/bar">
                      {viewMetric === 'both' ? (
                        <div className="flex w-full h-full items-end gap-[1px]">
                          <div 
                            className="flex-1 bg-emerald-500/40 rounded-t-[1px] transition-all duration-300" 
                            style={{ height: `${Math.min(100, currentHeight)}%` }}
                          />
                          <div 
                            className="flex-1 bg-indigo-500/40 rounded-t-[1px] transition-all duration-300" 
                            style={{ height: `${Math.min(100, powerHeight)}%` }}
                          />
                        </div>
                      ) : (
                        <div 
                          className={`w-full rounded-t-[2px] transition-all duration-300 ${viewMetric === 'current' ? 'bg-emerald-500/30' : 'bg-indigo-500/30'}`} 
                          style={{ height: `${Math.min(100, viewMetric === 'current' ? currentHeight : powerHeight)}%` }}
                        />
                      )}
                    </div>
                  );
              ...
              <div className="flex justify-between text-[9px] text-slate-500 font-mono mt-2 px-1">
                ...
                <span className="flex gap-4">
                  {(viewMetric === 'current' || viewMetric === 'both') && (
                    <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-emerald-500"></span>{(selectedComponent.current * 1000).toFixed(0)}mA</span>
                  )}
                  {(viewMetric === 'power' || viewMetric === 'both') && (
                    <span className="flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-indigo-500"></span>{(selectedComponent.power * 1000).toFixed(0)}mW</span>
                  )}
                </span>
                <span>100ms</span>
              </div>
